...
export const SchoolMemoView: React.FC<SchoolMemoViewProps> = ({
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      const [loadedLessons, loadedSettings] = await Promise.all([
        getSchoolLessons(),
        getSchoolClassSettings(),
      ]);
      if (cancelled) return;
  ...
  const classLessons = useMemo(
    () => lessons.filter((item) => item.classId === selectedClass),
  ...
  const saveExamScope = async () => {
    ...
    if (!editingId) {
      setDraft((current) => ({ ...current, examScopeSnapshot: examScopeInput }));
    }
  ...
  if (loading) {
    return (
      <div className="min-h-[100dvh] bg-[#f2f2f7] dark:bg-black flex items-center justify-center text-neutral-500">
        学校メモを読み込み中…
      </div>
    );
  }
              ...
              <button
                ...
                className={`min-h-11 rounded-xl font-bold transition-colors ${
                  selectedClass === classId
                    ...
                    : 'bg-white dark:bg-[#1c1c1e] border border-neutral-200 dark:border-neutral-700 text-neutral-700 dark:text-neutral-200'
                }`}
              >
                {classId}
              </button>
      ...
      <main className="max-w-3xl mx-auto px-4 py-4 space-y-4">
          ...
          <section className="rounded-2xl bg-amber-500/10 border border-amber-500/30 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-neutral-600 dark:text-neutral-300 mt-1 whitespace-pre-wrap">
                  {nextScheduledLesson.lessonContent || '授業内容はまだ未入力です'}
                </p>
              </div>
              <button
                type="button"
                onClick={() => editLesson(nextScheduledLesson)}
                className="min-h-11 px-3 rounded-xl bg-amber-500 text-black text-sm font-bold"
              >
                開く
              </button>
            </div>
        ...
        <section className="rounded-3xl bg-white dark:bg-[#1c1c1e] border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm">
          <div className="p-4 border-b border-neutral-200 dark:border-neutral-800">
            ...
            <button
              type="button"
              onClick={() => void saveExamScope()}
              className="mt-3 min-h-11 px-4 rounded-xl bg-neutral-900 dark:bg-white text-white dark:text-black font-bold text-sm inline-flex items-center gap-2"
            >
          ...
          <div className="p-4 space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs text-neutral-500">{editingId ? '授業メモを編集中' : '新しい授業メモ'}</p>
                <h2 className="text-xl font-bold">{selectedClass}・{formatDate(draft.lessonDate)}</h2>
              </div>
              <button
                type="button"
                onClick={() => startNewLesson()}
                className="min-h-11 px-3 rounded-xl border border-neutral-200 dark:border-neutral-700 text-sm font-semibold"
              >
                ＋ 新規
              </button>
            </div>
            ...
            <label className="block">
              ...
              <input
                ...
                className="mt-1.5 w-full min-h-11 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 text-base"
              />
            </label>
            ...
            {latestPreviousNext && (
              <button
                type="button"
                onClick={() => setDraft((current) => ({ ...current, lessonContent: latestPreviousNext }))}
                className="w-full min-h-11 rounded-xl bg-sky-500/10 border border-sky-500/30 text-sky-700 dark:text-sky-300 px-3 py-2 text-left flex items-start gap-2"
              >
                <Copy className="w-4 h-4 mt-0.5 shrink-0" />
                <span>
                  <strong>前回の「次回」を今回へコピー</strong>
                  <span className="block text-sm mt-0.5 whitespace-pre-wrap">{latestPreviousNext}</span>
                </span>
              </button>
            )}
            ...
            <label className="flex items-center gap-3 min-h-11 rounded-xl bg-neutral-100 dark:bg-neutral-900 px-3 cursor-pointer">
              <input
                ...
                onChange={(e) => setDraft((current) => ({ ...current, completed: e.target.checked }))}
                className="w-5 h-5 accent-amber-500"
              />
              <span className="font-semibold">この授業は実施済み</span>
            </label>
              ...
              {editingId && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (lesson) void toggleCompleted(lesson);
                  }}
                  className="min-h-12 px-4 rounded-xl border border-emerald-500/40 text-emerald-700 dark:text-emerald-300 font-semibold inline-flex items-center gap-2"
                >
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold">{selectedClass} の授業履歴</h2>
            <span className="text-xs text-neutral-500">{classLessons.length}件</span>
          </div>
